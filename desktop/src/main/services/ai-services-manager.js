...
function getPythonExecutable() {
    const servicePath = path.join(__dirname, '../../src/local-ai-service');
    ...
    try {
        ...
        logger.warn('python3 not found in PATH. AI service will not start.');
        return null;
    }
    ...
    const venvDir = path.join(servicePath, '.venv');
    const venvPython = path.join(venvDir, 'bin', 'python3');
    if (!fs.existsSync(venvPython)) {
        logger.info('No venv found, creating one...');
        try {
            const result = spawn('python3', ['-m', 'venv', venvDir], { cwd: servicePath });
            ...
            logger.error('Error creating virtual environment:', err);
        }
    }
